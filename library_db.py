...
def main():
    ...
    while True:
        res = 0
        print(f"What would you like to do with the database '{db_name}' [1/2]?")
        print(f" 1. Create a new database (this will delete any existing) '{db_name}')")
        print(f" 2. Use an existing databse '{db_name}'")
        init = input(">>")
        ...
        if init not in ("1", "2"):
            ...
            res += lf.create_all(session, db_name)
            if res != 0:
                return 1
            break
        elif init == "2":
